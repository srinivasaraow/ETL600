/*-------------------------------------------------------------------------*
 * $Workfile: 161_spi_SDRAM.asm $
 * Part of      : ETL600 / P4LT
 * Language     : asm
 * Created by   : Buhl, Michael PTUKT2
 * Remarks      :
 * Purpose      : Bootloader kernel for SPI with SDRAM settings  
 * Copyright    : ABB Switzerland Ltd., 2004
 *-------------------------------------------------------------------------*/
/*-------------------------------------------------------------------------*
 $Log: /Source/P4LT Prototype 2/DSP_Boot_Loader/SPI_Boot_SDRAM/161_spi_SDRAM.asm $
 * 
 * 1     19.01.05 15:30 Chchlee
 * 
 * 2     5.10.04 10:01 Buhl01
 * 
 * 1     11.07.04 18:20 Buhl01
 * initial
*-------------------------------------------------------------------------*/
...
//						PX=DM(I6,M6); // read word from internal scratch locations
        		   	PX = DM(0x40004);   // read word from internal scratch locations - new!		   	
        				R0=PX1;	        // swap px registers for external write
        				PX1=PX2;        // (see pag 5-13 of the 161 HW-Ref Rev3.0)
        				PX2=R0;
        				PM(I8,M14)=px;	//write word to external memory
						nop;
        			PX = DM(0x40005);  	// read word from internal scratch locations - new!      				
        				R0=PX1;	        // swap px registers for external write
        				PX1=PX2;        // (see pag 5-13 of the 161 HW-Ref Rev3.0)
        				PX2=R0;
        				PM(I8,M14)=px;	//write word to external memory
...
user_init:
		...
		ustat1=dm(SPICTL);
		bit clr ustat1 BAUDR15;	//clear DMA bit in SPICTL
		bit set ustat1 BAUDR2 | NSMLS;
		dm(SPICTL)=ustat1;

    			ustat1 = 0xAA11BB22;	// for Test use	
				dm(MSGR6)= ustat1;
