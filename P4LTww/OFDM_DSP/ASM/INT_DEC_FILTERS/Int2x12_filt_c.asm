 ...
 * FILE        : $Workfile: Int2x12_filt_c.asm $
 * COPYRIGHT   : 2004 by TL Consulting GmbH.
 * DESCRIPTION : 12 tap interpolator of complex signal by 2 
 * OPERATION   : 
 * PROTOTYPE   : void Int2x12_filt_c( complex_float    vOut[], 
 *                                   complex_float    vIn[],             
 *                                   int              nIn,               
 *                                   complex_float pm vCoeffs[2*6],       
 *                                   complex_float    vDelay[6+1] );    
 *
 * ASSUMPTIONS : all arrays starts with even address and there length is also even
 *               6 coefficients per stage ( overall 12 coefficients )
 *
 * REVISION HISTORY:
 * COMPLEXITY : 
 *-----------------------------------------------------------------------------
...
// i1 - address of output buffer vOut[]      
      ...
      // i9 - points coefficients of filter #1
      // f12 - output sample of filter #0
      // f13 - output sample of filter #1
      ...
      lcntr = r2, do fir until lce; /* loop per sample */
         ...
         // delay 2
         f8 = f0*f4, f13 = f9+f13, f0 = dm(i0,m2), f4=pm(i8,m8);    
         f5 = pm(i9,m8);
         f9 = f0*f5;
         f13 = f9+f13;
