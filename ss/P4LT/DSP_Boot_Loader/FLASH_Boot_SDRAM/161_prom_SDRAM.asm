 ...
 *-------------------------------------------------------------------------*/
/*-------------------------------------------------------------------------*
 $Log: /Source/P4LT Prototype 2/DSP_Boot_Loader/FLASH_Boot_SDRAM/161_prom_SDRAM.asm $
 * 
 * 1     19.01.05 15:30 Chchlee
 * 
 * 1     11.07.04 18:36 Buhl01
 *-------------------------------------------------------------------------*/
...
// ==========================  GET_ADDR  =============================================
// The processor jump-table holds the address of the boot-image for each DSP.
// (The image may also be relocated issuing the -p switch to the loader.)
// For non-MP systems (ID==0), the boot image resides at address 0x62A
// which is the first address after the kernel and processor jump-table.
// This subroutine executes once.
// ---------------------------------------------------------------------------
get_addr:       R0=DM(SYSTAT);
                R0=FEXT R0 BY 8:3;      // Determine this processor ID
                CALL read_prom_word;
                COMP(R0, R2);           // search for this processor's ID in jump table
                IF NE JUMP (PC,-2);  // If ID != fetch another ID keep looking
                DM(EIEP0)=R3;           // where this ID's data resides in PROM
// ========================= END GET_ADDR  ===========================================
